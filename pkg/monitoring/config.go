...
func ParseConfig() (Config, error) {
	...
	for i, feed := range feeds {
		...
		if err != nil {
			return cfg, fmt.Errorf("failed to parse program id '%s' from JSON at index i=%d with error: %w", feed.ContractAddressBase58, i, err)
		}
		transmissionsAccount, err := solana.PublicKeyFromBase58(feed.TransmissionsAccountBase58)
		if err != nil {
			return cfg, fmt.Errorf("failed to parse transmission account '%s' from JSON at index i=%d with error: %w", feed.TransmissionsAccountBase58, i, err)
		}
		stateAccount, err := solana.PublicKeyFromBase58(feed.StateAccountBase58)
		if err != nil {
			return cfg, fmt.Errorf("failed to parse state account '%s' from JSON at index i=%d with error: %w", feed.TransmissionsAccountBase58, i, err)
		}
