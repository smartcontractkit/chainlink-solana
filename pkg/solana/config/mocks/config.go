...
// ChainWriter provides a mock function with given fields:
func (_m *Config) ChainWriter() *config.ChainWriter {
	ret := _m.Called()

	var r0 *config.ChainWriter
	if rf, ok := ret.Get(0).(func() *config.ChainWriter); ok {
		r0 = rf()
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*config.ChainWriter)
		}
	}
