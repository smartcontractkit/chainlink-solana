...
func Test_decodeExtraArgs(t *testing.T) {
	...
	t.Run("decode extra args into map svm", func(t *testing.T) {
		...
		decodedSvmTag, err := hex.DecodeString(svmExtraArgsV1Tag)
		require.NoError(t, err)
		...
		require.NoError(t, err)
		output, err := extraDataDecoder.DecodeExtraArgsToMap(append(decodedSvmTag, buf.Bytes()...))
		require.NoError(t, err)
	...
	t.Run("decode extra args into map evm", func(t *testing.T) {
		...
		decodedEvmTag, err := hex.DecodeString(evmExtraArgsV2Tag)
		require.NoError(t, err)

		output, err := extraDataDecoder.DecodeExtraArgsToMap(append(decodedEvmTag, buf.Bytes()...))
		require.NoError(t, err)
