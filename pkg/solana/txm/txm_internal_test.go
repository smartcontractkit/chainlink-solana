...
func TestTxm_MultiNode(t *testing.T) {
	for _, eName := range []string{"fixed", "blockhistory"} {
		...
		t.Run("estimator-"+estimator, func(t *testing.T) {
			...
			// Enable MultiNode
			cfg := config.TOMLConfig{MultiNode: config.MultiNodeConfig{Enabled: true}}
			cfg.SetDefaults()
			...
			nodes := []mn.Node[mn.StringID, mocks.ReaderWriter]{
				mn.NewNode(cfg, mn.StringID("node1"), mc, "chainID", 0),
			}

			// TODO: Is this needed to test things?
			// TODO: Need to mock more of the ReaderWriter methods for TxSender??
			multiNode := mn.NewMultiNode(cfg,
				mn.NodeSelectionModeHighestHead,
				cfg.MultiNode.LeaseDuration(),
				nodes,
				[]mn.SendOnlyNode{},
				"chainID",
				"Solana",
