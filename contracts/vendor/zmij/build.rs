...
fn main() {
    ...
    // Treat unknown rustc as old (0) so we enable zmij_no_select_unpredictable in Docker/CI
    // where version detection may fail; select_unpredictable is only in Rust 1.88+.
    let rustc = rustc_minor_version().unwrap_or(0);
