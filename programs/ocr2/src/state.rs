...
pub struct State {
    pub nonce: u8,
    pub config: Config,
    pub oracles: [Oracle; 19], // sadly we can't use const https://github.com/project-serum/anchor/issues/632
...
pub fn config_digest_from_data(
    ...
    let mut data: Vec<&[u8]> = Vec::with_capacity(6 + 2 * oracles.len());
    let addr = contract_address.to_bytes();
    data.push(&addr);
    let count = config_count.to_be_bytes();
    data.push(&count);
    ...
    let offchain_version = offchain_config_version.to_be_bytes();
...
const_assert!(mem::size_of::<Config>() == 353); // bytes
...
mod tests {
    ...
    fn transmissions() {
        ...
        data.transmissions[524_279] = Transmission {
            answer: 1,
            timestamp: 1,
        };
