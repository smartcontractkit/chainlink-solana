...
export const wrapCommand = (command) => {
  return class Multisig extends SolanaCommand {
    ...
    execute = async () => {
      ...
      this.multisigState = await this.program.account.multisig.fetch(process.env.MULTISIG_ADDRESS)
      logger.debug('Multisig State:')
      logger.debug(JSON.stringify(this.multisigState, null, 4))
      ...
      this.proposal = this.flags.proposal
      const proposalState = await this.program.account.transaction.fetch(this.proposal)
      ...
      const remainingApprovalsNeeded = this.remainingApprovalsNeeded(proposalState, this.multisigState.threshold)
      const isReadyForExecution = remainingApprovalsNeeded > 0 ? false : true
      if (isReadyForExecution) {
        return this.wrapAction(this.executeProposal())
      }
    ...
    wrapAction = async (action) => {
      try {
        ...
        this.inspectProposalState(this.proposal)
        return {
          responses: [
            {
              tx: this.wrapResponse(tx, this.proposal.toString()),
              contract: this.proposal.toString(),
            },
          ],
    ...
    executeProposal = async (): Promise<string> => {
      logger.loading(`Executing proposal`)

      const proposalData = await this.program.account.transaction.fetch(this.proposal)
    ...
    inspectProposalState = async (proposal) => {
      ...
      if (remainingApprovalsNeeded <= 0) {
        logger.info(`Threshold has been met, an owner needs to run the command once more in order to execute it.`)
        return
      }
      // inverting the signers boolean array and filtering owners by it
      const remainingEligibleSigners = this.multisigState.owners.filter(
        (_, i) => proposalState.signers.map((s) => !s)[i],
      )
      logger.info(
