...
# When using an image with Rust 1.88+ (and edition2024 for constant_time_eq/blake3), set
# ANCHOR_BUILD_NO_VENDOR=1 and remove [patch.crates-io] from Cargo.toml and vendor/ to use crates.io only.
# See contracts/BUILD_WITHOUT_VENDOR.md.
if [ -z "${ANCHOR_BUILD_NO_VENDOR}" ]; then
